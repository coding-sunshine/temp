...
namespace App\Http\Controllers\Platform\Core\Helper;


use HipsterJazzbo\Landlord\Facades\Landlord;
use Illuminate\Support\Facades\Cache;
use App\Http\Controllers\Platform\User\Entities\Group;
use App\Http\Controllers\Platform\User\Entities\User;
...
class DataTableHelper
{
    ...
    public static function filterOwnerDropdown()
    {
        ...
        if (!empty(Landlord::getTenants()->first())) {
            ...
            $users = Cache::remember('users_for_account_'.$tenantId, 60, function(){
                return User::where('account_id', '=', Landlord::getTenants()->first())->get();
            });

            $groups = Cache::remember('groups_for_account_'.$tenantId, 60, function(){
                return Group::where('account_id', '=', Landlord::getTenants()->first())->get();
            });
        } else {
            $users = User::all();
            $groups = Group::all();
