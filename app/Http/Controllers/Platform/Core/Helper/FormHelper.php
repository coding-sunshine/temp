...
use Modules\Platform\Accounts\Http\Controllers\AccountContextController;
...
class FormHelper
{
    ...
    public static function shareWithChoises()
    {
        if (!empty(Landlord::getTenants()->first())) {
            $users = User::where('account_id', '=', Landlord::getTenants()->first())->get();
        } else {
            $users = User::all();
        }
    ...
    public static function assignedToChoises()
    {
        ...
        if(!empty(Landlord::getTenants()->first())){
            $users = User::where('account_id','=',Landlord::getTenants()->first())->get();
            $groups = Group::where('account_id','=',Landlord::getTenants()->first())->get();
        }else{
            $users = User::all();
            $groups = Group::all();
        }
