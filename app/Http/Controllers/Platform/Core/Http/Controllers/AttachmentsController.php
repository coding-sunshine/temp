...
class AttachmentsController extends AppBaseController
{
    ...
    public function uploadAttachments(AttachmentRequest $request)
    {
        ...
        if(!$user->isAdmin()){
            ...
            if($user->account != null && $user->account->storage_limit != null && $user->account->storage_limit > 0 ) {

                $allFileSize = $this->attachmentRepository->countFileSizeForAccount($user->account); // count file sizes

                if ($allFileSize >= $user->account->storage_limit) {
                    ...
                    return \Response::json([
                        'files' => [
                            [
                                ...
                                'name' => trans('core::core.you_exceeded_storage_please_switch_plan',['gigs' => $user->account->storage_limit]),
                                'size' => 1234,
                                'type' => 'png'
                            ]
                        ],
