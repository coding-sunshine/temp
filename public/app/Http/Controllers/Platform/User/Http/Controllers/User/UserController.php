...
use Modules\Platform\Accounts\Datatables\Scope\CurrentAccountScope;
...
class UserController extends SettingsCrudController
{
    ...
    protected $showFields = [
        'login_and_role' => [
            ...
            'account_id' => ['type' => 'manyToOne', 'relation' => 'account', 'column' => 'name','col-class'=>'col-lg-4', 'permission' => 'settings.access'],
    ...
    public function index()
    {
        ...
        $datatable->addScope(new CurrentAccountScope());
    ...
    public function store()
    {
        ...
        if(!$user->isAdmin() && !empty($user->account)){

            $userRepo = App::make(UserRepository::class);

            $usersCount = $userRepo->countUsersForAccount($user->account);

            if($usersCount >= $user->account->user_limit){
                flash(trans('user::users.you_cant_have_more_users',['planCount' => $user->account->user_limit,'currentCount' => $usersCount]))->error();
